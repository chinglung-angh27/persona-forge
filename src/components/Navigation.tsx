...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const navItems: { id: ViewMode; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'persona', label: 'My Persona', icon: <UserCheck className="w-5 h-5" /> },
    { id: 'dna', label: 'DNA', icon: <Fingerprint className="w-5 h-5" /> },
    { id: 'daily', label: 'Daily Mode', icon: <Calendar className="w-5 h-5" /> },
    { id: 'simulator', label: 'Simulator', icon: <Cpu className="w-5 h-5" /> },
    { id: 'habits', label: 'Habits', icon: <CheckSquare className="w-5 h-5" /> },
    { id: 'reflections', label: 'Reflections', icon: <Brain className="w-5 h-5" /> },
    { id: 'evolution', label: 'Evolution', icon: <TrendingUp className="w-5 h-5" /> },
    { id: 'references', label: 'References', icon: <Bookmark className="w-5 h-5" /> },
  ...
  return (
    <>
      ...
      <header className="md:hidden fixed top-0 left-0 w-full z-40 bg-[#121212] shadow-[-6px_-6px_12px_rgba(255,255,255,0.03),6px_6px_12px_rgba(0,0,0,0.5)] flex justify-between items-center h-20 px-6 border-b border-[#1c1b1b]">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="w-10 h-10 rounded-full bg-[#121212] neo-extruded flex items-center justify-center overflow-hidden border border-[#2a2a2a]"
            title="Profile"
          >
            <img 
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 rounded-t-3xl bg-[#121212] shadow-[0_-8px_16px_rgba(0,0,0,0.7)] border-t border-[#1e1e1e] flex justify-around items-center px-3 py-2 h-20">
        {[
          { id: 'dashboard' as ViewMode, label: 'Home', icon: <Home className="w-5 h-5" /> },
          { id: 'dna' as ViewMode, label: 'DNA', icon: <Fingerprint className="w-5 h-5" /> },
          { id: 'daily' as ViewMode, label: 'Daily', icon: <Calendar className="w-5 h-5" /> },
          { id: 'simulator' as ViewMode, label: 'Sim', icon: <Cpu className="w-5 h-5" /> },
          { id: 'references' as ViewMode, label: 'Refs', icon: <Bookmark className="w-5 h-5" /> },
          { id: 'settings' as ViewMode, label: 'Settings', icon: <Settings className="w-5 h-5" /> },
        ].map((tab) => {
