---
name: tdd-repair
description: Test-Driven Repair — given a failing test, spawn a bounded headless `claude -p` (Read/Edit/Bash only) that makes the test pass without modifying it. Bounded cost via --max-budget-usd, bounded capability via --allowedTools. Closes the loop the TDD plugins didn't — we generate tests, this fixes the code to satisfy them.
...
> Installed standalone in this project (ruflo marketplace). The bundled script
> lives at `scripts/tdd-repair/tdd-repair.mjs` next to this file — no ruflo
> runtime, binary, or MCP required. It shells out to `claude -p`, which is
> already in our toolchain.
...
- Local TDD workflow — write the failing test first, then run `tdd-repair` to drive the green.
- Regression triage — a previously-green test went red; before opening an issue, spend ~$1 to see if the fix is trivial.

## When NOT to use

- **No failing test exists.** Write a failing test first.
- **Architectural changes.** This skill is for tactical "make red green" fixes. Cross-module refactors that incidentally break tests should be done by a human.
...
| Confirmation gate | `--confirm` REQUIRED — without it, returns dry-run plan. |
